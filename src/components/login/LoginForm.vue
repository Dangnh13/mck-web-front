                                ...
                                <v-col cols="12" sm="8">
                                    <ValidationProvider rules="required|email" name="email" v-slot="{ errors }">
                                        <v-text-field v-model="email" label="Email" outlined dense color="blue"
                                            autocomplete="false" class="mt-16" :error-messages="errors" />
                                    </ValidationProvider>
                                    <ValidationProvider rules="required" name="password" v-slot="{ errors }">
                                        <v-text-field v-model="password" label="Password" outlined dense color="blue"
                                            autocomplete="false" type="password" class="mt-16"
                                    ...
                                    <v-row>
                                        ...
                                        <v-col cols="12" sm="5">
                                            <span class="caption blue--text">Forgot
                                                password</span>
                                        </v-col>
                                    </v-row>
                                    <v-btn color="blue" dark block tile type="submit">Log in</v-btn>

                                    <h5 class="text-center  grey--text mt-4 mb-3">Or Log in
                                        using</h5>
            ...
            <v-col cols="12" md="6" class="blue rounded-bl-xl">
                <div style="  text-align: center; padding: 180px 0;">
                    <v-card-text class="white--text">
                        ...
                        <h6 class="text-center">Let's get you all set up so you can
...
export default {
    ...
    methods: {
        loginHandling() {
            console.log("vao login handling");
            this.$refs.loginFormRef.validate().then(success => {
                if (success) {
                    alert("Submit sucecss");
                    return true;
                }
                return false;
            });
